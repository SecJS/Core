...
import { MakeResponse } from '../../../src/Responses'

test.group('MakeResponse', group => {
  group.beforeEach(() => {
    // ioc.restore();
  })
  ...
  test('should return a collection of objects', async assert => {
    ...
    const response = new MakeResponse().withCollection(data);
  ...
  test('should return one object', async assert => {
    ...
    const response = new MakeResponse().withOne(data, 'I can pass messages or not!!');
  ...
  test('should return just a status with message', async assert => {
    const response = new MakeResponse().withoutBody('Without body!!');
  ...
  test('should return an error message', async assert => {
    ...
    const response = new MakeResponse().withError(data, 'A very nice error message :P', 'ERROR_CODE', 400);
  ...
  test('should return an error message with validation', async assert => {
    ...
    const response = new MakeResponse().withValidationError(data, validation, 'A very nice error message :P', 'WRONG_ARGS', 400);
